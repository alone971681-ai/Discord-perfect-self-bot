...
def start_bot():
    try:
        ...
        flash("Discord selfbot started successfully!", "success")
...
def bot_status():
    ...
    try:
        ...
        return json.dumps({"error": str(e), "bot_status": BOT_STATUS})
...
def setup_24_7():
    ...
    # Build public URL — works on Replit, Railway, Render, VPS, etc.
    # Priority: APP_URL env var → Replit env → fallback placeholder
    monitor_url = (
        os.environ.get('APP_URL')
        or (
            f"https://{os.environ['REPL_SLUG']}.{os.environ['REPL_OWNER']}.repl.co"
            if os.environ.get('REPL_SLUG') and os.environ.get('REPL_OWNER')
            else "http://your-server-ip:5000"
        )
    )
